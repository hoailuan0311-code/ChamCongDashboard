...
function showBanner() {
    ...
    setTimeout(() => banner.remove(), 8500);
}
showBanner();
setInterval(showBanner, 30000);  // ← lặp mỗi 30s mãi mãi
...
createFlag(
    ...
    "calc(3vw + clamp(65px,8.5vw,108px))", 0.5
);
...
function buildTankSVG(bodyColor, darkColor, label) {
    ...
    return `<svg width="210" height="86" viewBox="0 0 210 86" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x="97" y="56" text-anchor="middle" font-size="10.5"
              fill="rgba(255,255,200,0.88)" font-family="monospace" font-weight="bold">
            ${label}
        </text>
    </svg>`;
}
...
function spawnTank(label, bodyColor, darkColor, delay, duration) {
    ...
    // Xóa DOM sau khi animation xong, KHÔNG xóa interval
...
spawnTankWave();                         // chạy ngay lập tức
setInterval(spawnTankWave, 22000);       // ← lặp mỗi 22s mãi mãi
...
function drawFX() {
    fx.clearRect(0, 0, cvs.width, cvs.height);   // trong suốt hoàn toàn
    ...
    requestAnimationFrame(drawFX);   // ← loop mãi mãi
}
drawFX();
